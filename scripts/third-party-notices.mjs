 ...
 * bundles with dozens of libraries inlined, and nothing in the tree says so.
 *
 * The list is built from esbuild's own metafiles rather than from
 * `package.json` — a dependency list is what was *installed*, and the thing
 * that needs attributing is what was **bundled**. The two differ in both
 * directions: `sharp` is a dependency and is deliberately left external, and
 * a transitive package nobody declared can still end up inside the bundle.
 */
 ...
 */
export async function thirdPartyNotices(builds) {
  ...
  for (const build of builds) {
    for (const input of Object.keys(build.metafile.inputs)) {
      ...
      if (owner !== null) {
        directories.set(owner.name, owner.dir);
      }
    }
  }
...
function render(packages) {
  const lines = [
    ...
    '`worker/index.js` and `cli/index.js`. Each remains under its own licence,',
    'reproduced here in full where the package ships the text. Mallok itself is',
