...
export async function upgradeProject(
  ...
  // What has to go back if anything fails. The lockfile is read as bytes and
  ...
  const lockBefore = await readFile(lockPath).catch(() => null);
  ...
  const restore = async (): Promise<void> => {
    ...
    // Reinstalled, not merely rewritten: `node_modules` holds the target at
    // this point, and a project whose files say one version while its
    // installed tree holds another fails in a way nobody can read.
    const back = await runner('npm', installArgs(), { cwd: projectDir });
    if (back.code !== 0) {
      throw new CliError(
        EXIT.user,
        `Rolled back to ${from}, but reinstalling it failed.`,
        `${lastLine(back.stderr, back.stdout)} — package.json and the ` +
          'lockfile are back as they were; run `npm install` to finish.',
      );
    }
  };
