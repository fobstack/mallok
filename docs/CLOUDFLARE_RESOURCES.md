...
One configuration file per site: the project's **own** `wrangler.jsonc`, at
its root. `mallok create` copies the template into the new project and then
edits that file in place (`applyResourceIds`, `src/cli/create.ts`), filling in
the Worker name, the database name and id, the bucket name and — when a domain
was given — the `custom_domain` routes. `wrangler deploy` then runs inside the
project with no `-c` flag.

It used to write a second, nested config to `.mallok/sites/<slug>.jsonc` and
deploy with `-c`. That form is gone. wrangler resolves `main` and
`assets.directory` relative to **the config file's own location**, so every
relative path in a nested config has to be rewritten, and missing one made the
deploy fail with "entry-point file was not found" (found running Gate A for
real, 2026-09-03; `docs/tasks/TASK-01.md §5`). A config that sits where its
paths were written for cannot have that bug at all.

An example of the file after `mallok create` has edited it:
...
The order is the safety property, not an implementation detail: **everything
free and reversible happens before anything on the account changes.** The
command used to sign in, create a database, create a bucket, write a config
and only then attempt a deploy — so a project that could not build left two
real resources behind, named after a site that did not exist, recorded
nowhere.

Free, reversible, and in a directory the user chose:

1. Check the target directory is absent or empty, and the slug is a legal
   resource name. The directory argument and the Cloudflare slug are separate;
   the slug defaults to the directory's own name and `--slug` overrides it.
2. Check the packaged template is intact.
3. Generate the project.
4. `pnpm install --frozen-lockfile` inside it.
5. `pnpm run build` inside it.
6. `wrangler deploy --dry-run` using the project's **own** Wrangler
   (`node_modules/.bin/wrangler`), never `npx wrangler` — `npx` resolves to
   whatever the registry publishes today, which is not the version the project
   was built against.

`--dry-run` stops here and removes the directory. `--no-deploy` stops here and
keeps it. Neither has touched Cloudflare.

Only then, and each step recorded in `.mallok/create-state.json` **before** the
next one runs:

7. `wrangler whoami` — confirm the OAuth session. The CLI never asks the user
   to mint an account token by hand.
8. Create D1 `mallok-<slug>-db`, take its `database_id`, record it.
9. Create the R2 bucket `mallok-<slug>-media`, record it.
10. Edit the project's own `wrangler.jsonc` with the real names and ids.
11. `wrangler deploy`, record the origin.
12. Generate `MALLOK_SECRET` (32 random bytes) and pipe it straight to
    `wrangler secret put`. It is never written to a file, never printed, and
    never recorded in the ledger.
13. Register the site in the project's `.mallok/sites.json`, which is what
    `mallok publish` and `mallok destroy` read later.
14. If a domain was given: the configuration carries the `custom_domain`
    routes, so the deploy creates the DNS records and certificate. Then attach
    the R2 custom domain `media.<domain>`, create the Turnstile widget for that
    hostname and store its key in D1, and tell the user which exact permissions
    `CF_API_TOKEN` needs so they can create it in the dashboard and
    `wrangler secret put` it.
15. Open `https://mallok-<slug>.<account>.workers.dev/_mallok/setup` (or the
...
- **Local**: `pnpm dev` in the project (`wrangler dev --local`), with D1 and
...
One file per project, written after a successful deploy: it holds the slug,
the origin, the domain, the database id, the bucket name and the rate-limit
namespace, and **never a secret**, because the file is meant to be committed.
