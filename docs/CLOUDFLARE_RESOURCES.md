...
`wrangler deploy -c .mallok/sites/<slug>.jsonc`. wrangler resolves `main` and
`assets.directory` relative to **this file's own location**, not the working
directory, so `buildSiteConfig` (`src/cli/provision.ts`) rewrites both to
`../../<path>` — two levels back up to the repository root, matching where
`.mallok/sites/` sits. Copying the repository's own `wrangler.jsonc` in
unmodified fails to deploy for exactly this reason (found running Gate A for
real, 2026-09-03; `docs/tasks/TASK-01.md §5`):
