...
- 计划公开仓库：`github.com/JasonYv/mallok`（尚未发布）

## 1. 产品定义

Mallok 是一套内容网站产品。长期目标用户包括第一次做网站的人和非技术内容创作者；当前 MVP 先由开发者和技术型站点负责人通过 Markdown 与 CLI 验证内容内核、模板、static target、D1 更新和安全部署链路。

Mallok 的目标，是在“内容站从模板到持续发布”这条窄路径上，做出比通用站点框架更轻、更容易完成的产品；它不复制 Astro 的组件生态，也不声称在所有 Web 场景取代 Astro。比较性结论必须满足 [产品愿景](PRODUCT_VISION.md) 的工程基准与真人任务证据门。
...
- Node、包管理器、组件语法、render mode、adapter、云绑定和失败恢复形成连续认知门槛，普通内容创作者无法独立完成首次上线；
- 许多工具提供零散能力，却仍要求用户自己拼接模板、CMS、数据库、部署和恢复路径。

Mallok 要把这些边界变成明确、可测试的产品契约，并通过默认值、引导流程和逐层暴露复杂度，让基础用户不必理解内部技术名词。

## 3. 目标用户

### 3.1 MVP 首发操作者

会安装 Node.js、使用终端和编辑 Markdown 的独立开发者或技术型站点负责人。他们希望快速建立博客、文档站、新闻站或产品内容站，并保留对 HTML、CSS、路由和部署的控制权。

### 3.2 长期核心用户

不会前端框架、Node.js、Git 或 Cloudflare 配置的内容创作者、小微企业站主和团队编辑。MVP 尚不能承诺他们独立完成建站；长期必须通过 Mallok Studio 或受控引导入口完成模板选择、内容编辑、预览、发布和恢复，而不要求使用终端。

### 3.3 小型内容团队

MVP 由开发者负责主题与部署，编辑可以提交 Markdown；长期编辑通过 Studio 完成日常内容工作，技术负责人保留 Git/CLI 审计与自动化能力。
...
1. 从官方模板创建站点，只填写站点本身的信息，不先学习框架内部结构。
2. 五分钟内创建一个可预览的 developer-path 站点。
3. 添加一篇 Markdown 文章并看到本地页面更新。
4. 替换主题，而不迁移文章数据。
5. 构建一个无需服务端的静态目录。
6. 切换到 D1 模式，让已发布文章无需重新构建即可更新。
7. 用一条部署命令完成构建、检查、迁移提示和 Worker 发布。
8. 在失败时得到可执行的诊断和恢复动作，而不是底层堆栈。
9. 长期通过 Studio 完成“选模板 → 填站点信息 → 写/导入内容 → 预览 → 发布”，不暴露框架、SQL、Wrangler 或部署状态机。
...
### 5.4 MVP 先交付 CLI，长期多界面共享同一应用能力

CLI 是 MVP 的第一交付界面和长期稳定自动化接口，不是大众易用性的终点。CLI、未来 Guided Start 和 Studio 必须调用同一 application/domain service 或管理 API；Studio 不得 shell out 到 CLI，也不拥有独立业务真相。内核、CLI 和远程内容更新达到稳定验收后再实现浏览器身份、托管和可视化编辑，避免用长期 bearer token 或直接写 D1 换取表面简单。

该接口分层与阶段裁决见 [ADR-0006](adr/0006-guided-product-surface.md)。
...
- Mallok Studio：长期核心产品界面，分阶段提供浏览器引导初始化、模板选择与预览、可视化 Markdown 编辑、发布状态、可理解错误和恢复入口；完整实现前需要新的身份、权限、作者真相、托管与威胁模型 PRD/ADR。
- 草稿预览链接和多人角色权限。
- 图片上传与 R2 媒体库。
- 主题脚手架、主题市场和在线预览。
- 增量静态构建与更细粒度缓存失效。
...
- 运行不受信任的远程主题代码。
- 承诺静态站在 D1 更新后自动变化。
- 把 Mallok 宣传为适用于所有 Web 应用的 Astro 替代品。
...
MVP release candidate 的工程发布门由下列后四项及对应 AC 决定，不以 GitHub star 为主。第一项只验证 §3.1 的 developer path：它由产品负责人在公开 MVP 后 30 天内组织，**不阻断 T-012 release candidate**，也不得拿自动化 CI 时长冒充，更不能据此声称非技术用户已经能独立使用：
...
Phase 4 开始前必须另立非技术用户研究与验收：无 Node/终端经验用户的模板选择、创建、编辑、预览、首次公开发布和错误恢复闭环。目标完成率、时间、求助和手工配置次数以 [产品愿景 §5.2](PRODUCT_VISION.md#52-phase-4-non-technical-path) 为基线；没有无终端入口时不得开展或宣称该测试通过。

“比 Astro 更轻”同样不是 MVP 自动成立的结论。需要锁定双方版本，在同一内容、机器和脚本下记录安装体积/时间、依赖数、cold/warm build、峰值 RSS、输出 bytes、所需命令与手工配置，规则见 [产品愿景 §5.3](PRODUCT_VISION.md#53-与-astro-的可复现对比)。
