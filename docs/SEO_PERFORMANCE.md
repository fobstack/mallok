...
Both columns are enforced, and neither substitutes for the other. Lighthouse
CI can only assert one threshold against the median of its runs, so the
release gate used to assert the floor (0.90) alone and describe the median
requirement as a rule it "cannot express" — which in practice meant it was no
longer being checked. `scripts/lighthouse-gate.mjs` reads the three reports
`lhci` writes and asserts both, per URL, printing every run's score
(`docs/RELEASE_GATE.md §14`).
